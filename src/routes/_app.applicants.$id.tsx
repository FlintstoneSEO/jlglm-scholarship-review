      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            {a.is_finalist && (
              <Badge className="bg-gold/20 text-gold-foreground border-gold/40">Finalist</Badge>
            )}
            {a.is_selected && (
  ...
  return (
    <div className="rounded-lg border border-border p-4 flex items-center justify-between">
      <div className="flex items-center gap-3">
        <div
          className={`h-9 w-9 rounded-lg grid place-items-center ${present ? "bg-success/15 text-success" : "bg-destructive/10 text-destructive"}`}
        >
...
function ScoringPanel({
  ...
  const committeeNotes = notes
    .filter((n) => n.note_type === "committee" || n.note_type === "general")
    .slice(0, 5);
  ...
  const checklist = [
    ...
    {
      label: "Guardian signature",
      ok: !!applicant.guardian_signature_status || !!applicant.is_18_or_older,
    },
  ];
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="lg:col-span-2 space-y-4">
          <Card className="p-5 rounded-xl border-border/60">
            ...
            <div className="mt-4 space-y-1.5 text-sm">
              ...
              {applicant.phone && (
                <div className="flex items-center gap-2 text-muted-foreground">
                  ...
                  <span className="text-foreground">{applicant.phone}</span>
                </div>
              )}
              {applicant.college_attending && (
                <div className="text-muted-foreground">
                  College: <span className="text-foreground">{applicant.college_attending}</span>
                </div>
              )}
              {applicant.graduation_high_school && (
                <div className="text-muted-foreground">
                  High school:{" "}
                  <span className="text-foreground">{applicant.graduation_high_school}</span>
                </div>
              )}
            </div>
            ...
            <div className="mt-4">
              ...
              <ul className="space-y-1.5 text-sm">
                {checklist.map((c) => (
                  <li key={c.label} className="flex items-center gap-2">
                    <span
                      className={`h-5 w-5 rounded grid place-items-center ${c.ok ? "bg-success/15 text-success" : "bg-destructive/10 text-destructive"}`}
                    >
              ...
              {miss.length > 0 && (
                <div className="mt-3 text-xs text-warning">Missing: {miss.join(", ")}</div>
              )}
            </div>
          </Card>
        ...
        <div className="lg:col-span-3 space-y-4">
          <Card className="p-6 rounded-xl border-border/60">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Justice League Rubric · Writing + Rhetoric
                </p>
              </div>
              {mine &&
                (mine.is_complete ? (
            ...
            <div className="mt-3 flex items-center justify-between rounded-lg bg-primary text-primary-foreground px-4 py-3">
              <div>
                ...
                <div className="font-display text-2xl leading-tight">
                  {subtotal} <span className="text-sm opacity-90">/ {MAX_REVIEWER_SCORE}</span>
                </div>
              </div>
              <div className="text-right text-[11px] opacity-95 max-w-[200px] leading-snug">
                Combined applicant score = sum of all 5 reviewer subtotals (max {MAX_COMBINED_SCORE}
                ).
              </div>
            </div>
            ...
            <Accordion type="single" collapsible className="mt-3">
              <AccordionItem
                value="guide"
                className="border border-gold/40 bg-gold/10 rounded-lg px-3"
              >
                <AccordionTrigger className="text-sm font-semibold text-foreground hover:no-underline py-3">
                  <span className="flex items-center gap-2">
                    ...
                    <Badge
                      variant="outline"
                      className="ml-1 border-gold/50 text-[10px] uppercase tracking-wide"
                    >
                      Help
                    </Badge>
                  </span>
                  ...
                  <div className="space-y-3 text-xs pt-1">
                    <RubricGuide
                      ...
                      tiers={[
                        {
                          range: "0–1",
                          desc: "Does not address the question, and/or poor grammar and structure impedes understanding.",
                        },
                        {
                          range: "2–4",
                          desc: "May overlook aspects of the question, and/or grammar and structure interfere with understanding.",
                        },
                        {
                          range: "5–7",
                          desc: "Addresses the question, and grammar and structure do not interfere with understanding.",
                        },
                        {
                          range: "8–9",
                          desc: "Addresses the question and writing is clear with proper grammar.",
                        },
                      ]}
                    />
                    <RubricGuide
                      ...
                      tiers={[
                        {
                          range: "0–1",
                          desc: "No personal experience or examples are related to the question and no reasoning is addressed.",
                        },
                        {
                          range: "2–4",
                          desc: "Little personal experience or examples related to the question; insufficient argument for their definition.",
                        },
                        {
                          range: "5–7",
                          desc: "Personal experience or examples are related to the question, and response makes an argument for their definition.",
                        },
                        {
                          range: "8–9",
                          desc: "Personal experience or examples relate to the question, and the response demonstrates a strong argument for their definition.",
                        },
                      ]}
                    />
                  </div>
            ...
            <div className="mt-3">
              ...
              <Textarea
                ...
                placeholder="Strengths, concerns, things to discuss…"
              />
            </div>

            <div className="mt-5 flex flex-wrap items-center justify-end gap-2">
              <Button onClick={() => save(false)} disabled={busy || !canEdit} variant="outline">
                Save draft
              </Button>
              <Button
                onClick={() => save(true)}
              ...
              >
                {mine?.is_complete ? "Update Review" : "Submit Review"}
              </Button>
            </div>
          </Card>
        </div>
      ...
      {/* Individual reviewer scores are visible only to administrators. */}
      {showAllReviews && (
        <Card className="p-6 rounded-xl border-border/60">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {summary.completed} of {REVIEWERS_PER_APPLICANT} completed · Combined{" "}
                {summary.combined} / {MAX_COMBINED_SCORE}
              </p>
            </div>
            ...
            <table className="w-full text-sm">
              <thead className="text-xs uppercase tracking-wider text-muted-foreground">
                <tr>
                  ...
                  <th className="text-right py-2">Writing</th>
                  <th className="text-right py-2">Rhetoric</th>
                  <th className="text-right py-2">Subtotal</th>
                  <th className="text-left py-2 pl-3">Recommendation</th>
                  <th className="text-left py-2 pl-3">Status</th>
                  <th className="text-left py-2 pl-3">Submitted</th>
                </tr>
              </thead>
...
function ReviewerDiscussionDocumentsPanel({
  applicantId,
  userId,
  userEmail,
  canUpload,
  ...
  const { data: docs = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("reviewer_discussion_documents")
        .select("*")
  ...
  async function upload(file?: File) {
    ...
    if (file.size > 10 * 1024 * 1024)
      return toast.error("Unable to upload file. Please try again.");
    ...
    const ins = await supabase.from("reviewer_discussion_documents").insert({
      applicant_id: applicantId,
      reviewer_id: userId,
  ...
  async function openDoc(path: string) {
    const { data, error } = await supabase.storage
      .from("reviewer-discussion-documents")
  ...
  return (
    <div className="md:col-span-2 rounded-lg border border-border p-4 space-y-3">
      ...
      <p className="text-xs text-muted-foreground">
        Upload a marked-up PDF or Word document if you edited the applicant essay for team
        discussion. The original essay submission will remain unchanged.
      </p>
      {canUpload && (
        <Input
          type="file"
      ...
      {docs.length === 0 ? (
        <p className="text-sm text-muted-foreground">No edited copies have been uploaded yet.</p>
      ) : (
          ...
          {docs.map((d) => (
            <div key={d.id} className="flex items-center justify-between border rounded p-2">
              ...
              <div className="flex items-center gap-2">
                ...
                {isAdmin && (
                  <Button size="sm" variant="destructive" onClick={() => deleteDoc(d)}>
                    Delete
                  </Button>
                )}
              </div>
...
function RubricGuide({
  ...
  title: string;
  tiers: { range: string; desc: string }[];
}) {
  return (
    <div>
...
function NotesPanel({
  ...
  return (
    <div className="grid lg:grid-cols-3 gap-5">
        ...
        <Card className="p-6 rounded-xl border-border/60">
          ...
          <div className="mt-4 space-y-3">
            ...
            <Textarea
              ...
              placeholder="Write a note for the committee…"
            />
      ...
      <Card
        className={`p-6 rounded-xl border-border/60 ${canEdit ? "lg:col-span-2" : "lg:col-span-3"}`}
      >
        <h3 className="font-display text-lg">Note History</h3>
        {notes.length === 0 ? (
          <p className="text-sm text-muted-foreground mt-3">No notes yet.</p>
        ) : (
          <div className="mt-4 space-y-3">
            {notes.map((n) => (
    ...
    <div className="grid lg:grid-cols-3 gap-5">
      {canEdit && (
        <Card className="p-6 rounded-xl border-border/60">
          ...
          <p className="text-xs text-muted-foreground">
            Use Contact Center to send branded templates.
          </p>
          <div className="mt-4 space-y-3">
            <div>
              <Label>Type</Label>
            ...
            <Textarea
              rows={4}
              placeholder="Message or summary"
              value={message}
      ...
      >
        ...
        {contacts.length === 0 ? (
          <p className="text-sm text-muted-foreground mt-3">No contact yet.</p>
        ) : (
          <div className="mt-4 space-y-3">
            {contacts.map((c) => (
              <div key={c.id} className="rounded-lg border border-border p-4">
                <div className="flex justify-between text-xs text-muted-foreground">
                  ...
                  <span>{new Date(c.contacted_at).toLocaleString()}</span>
                </div>
