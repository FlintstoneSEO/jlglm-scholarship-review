...
function ContactCenter() {
  ...
  async function logSend() {
    ...
    const { error } = await supabase.from("contact_logs").insert({
      applicant_id: applicant.id, contact_type: "email", subject: renderedSubject, message: renderedBody,
      contacted_by: user?.id, contacted_by_name: user?.email,
    });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">Compose branded messages, personalize them, and log every contact attempt.</p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        <Card className="p-6 rounded-xl border-border/60 space-y-4">
          <div>
            ...
            <Select value={applicantId} onValueChange={setApplicantId}>
              ...
              <SelectContent className="max-h-72">
                {apps.map((a) => <SelectItem key={a.id} value={a.id}>{fullName(a)} {a.email ? `· ${a.email}` : ""}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div>
          ...
          <div><Label>Message</Label>
            ...
            <p className="text-xs text-muted-foreground mt-1">Variables: <code>{`{{first_name}} {{last_name}} {{missing_items}}`}</code></p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button onClick={openMailto} disabled={!applicant?.email}><Mail className="h-4 w-4 mr-1.5" /> Open in mail app</Button>
            <Button variant="outline" onClick={copyAll}><Copy className="h-4 w-4 mr-1.5" /> Copy</Button>
            <Button variant="outline" onClick={logSend} disabled={!applicant}><Send className="h-4 w-4 mr-1.5" /> Log as sent</Button>
          </div>
        </Card>
        ...
        <Card className="p-6 rounded-xl border-border/60">
          ...
          <div className="mt-4 rounded-lg border border-border bg-card p-5 whitespace-pre-wrap text-sm leading-relaxed">{renderedBody}</div>
        </Card>
      </div>
    </div>
  );
