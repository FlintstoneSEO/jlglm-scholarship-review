...
const corsHeaders = {
  ...
  "Access-Control-Allow-Headers": "authorization, apikey, content-type, x-jlgl-sync-secret",
};
...
const documentFields = {
  lara_documentation: {
    ...
    documentType: "lara_documentation",
  },
  profit_loss_2024: { label: "2024 Profit & Loss Statement", documentType: "profit_loss_2024" },
  profit_loss_2025: { label: "2025 Profit & Loss Statement", documentType: "profit_loss_2025" },
} as const;
...
function parseSpreadsheetId(input: string) {
  const match =
    input.match(/\/spreadsheets\/d\/([a-zA-Z0-9_-]+)/) ?? input.match(/^([a-zA-Z0-9_-]{20,})$/);
  return match?.[1] ?? null;
}
...
async function googleAccessToken() {
  ...
  const response = await fetch("https://oauth2.googleapis.com/token", {
    ...
    body: new URLSearchParams({
      ...
      assertion: `${encodedHeader}.${encodedPayload}.${base64Url(signature)}`,
    }),
  });
  if (!response.ok)
    throw new Error("Google authorization failed. Confirm the service-account secret.");
...
async function readSheet(spreadsheetId: string, worksheetName?: string | null) {
  ...
  if (!metadataResponse.ok)
    throw new Error(
      "Google could not open this spreadsheet. Share it with the service account as Viewer.",
    );
  const metadata = (await metadataResponse.json()) as {
    sheets?: { properties: { sheetId: number; title: string } }[];
  };
  const sheet = worksheetName
    ? metadata.sheets?.find((item) => item.properties.title === worksheetName)
    : metadata.sheets?.[0];
  if (!sheet) throw new Error("The selected worksheet was not found.");
  const valuesResponse = await fetch(
    `https://sheets.googleapis.com/v4/spreadsheets/${encodeURIComponent(spreadsheetId)}/values/${encodeURIComponent(sheet.properties.title)}?majorDimension=ROWS`,
    { headers },
  );
  if (!valuesResponse.ok)
    throw new Error("Google could not read rows from the selected worksheet.");
  ...
  return {
    serviceAccountEmail,
    worksheets: (metadata.sheets ?? []).map((item) => ({
      name: item.properties.title,
      gid: String(item.properties.sheetId),
    })),
    worksheet: { name: sheet.properties.title, gid: String(sheet.properties.sheetId) },
    columns,
    rows,
  };
}
...
function sourceRecord(headers: string[], row: unknown[], mappings: Mapping[]) {
  ...
  const applicantName =
    [mapped.applicant_first_name, mapped.applicant_middle_name, mapped.applicant_last_name]
      .map(text)
      .filter(Boolean)
      .join(" ") ||
    text(mapped.applicant_name) ||
    text(mapped.contact_name);
  const businessName = text(mapped.business_name);
  const usesLiveNameFields = mappings.some(
    (mapping) =>
      mapping.target_field === "applicant_first_name" ||
      mapping.target_field === "applicant_last_name",
  );
  if (usesLiveNameFields && !text(mapped.applicant_first_name))
    missing.push("applicant_first_name");
  if (usesLiveNameFields && !text(mapped.applicant_last_name)) missing.push("applicant_last_name");
...
async function syncSource(
  ...
  const { data: batch } = source.created_by
    ? await admin
        .from("import_batches")
        .insert({
          program_id: source.program_id,
          source: "google_sheets",
          source_file_name: source.spreadsheet_url,
          imported_by: source.created_by,
        })
        .select("id")
        .single()
    : { data: null };
  ...
  try {
    ...
    const mappedColumns = new Set(mappings.map((mapping) => mapping.source_column));
    const unmappedColumns = sheet.columns.filter((column) => !mappedColumns.has(column));
    const logs: {
      batch_id: string;
    ...
    for (const [index, row] of sheet.rows.entries()) {
      ...
      try {
        ...
        if (record.missing.length)
          throw new Error(`Missing required mapped fields: ${record.missing.join(", ")}`);
        ...
        if (applicationError || !application)
          throw applicationError ?? new Error("Application upsert returned no record.");
        ...
        for (const [key, value] of Object.entries(record.mapped)) {
          ...
          details[key] =
            key === "year_established" || key === "employee_count" || key === "amount_requested"
              ? numberValue(value)
              : key === "eligibility_answers"
                ? { value: text(value) }
                : text(value);
        }
        ...
        for (const [target, metadata] of Object.entries(documentFields)) {
          ...
          const { data: existingDocument } = await admin
            ...
            .eq("document_type", metadata.documentType)
            .maybeSingle();
          if (!urls.length) {
            if (existingDocument) {
          ...
          if (existingDocument) {
            const { error: updateDocumentError } = await admin
              .from("application_documents")
              .update({ label: metadata.label, external_url: url })
              .eq("id", existingDocument.id);
            if (updateDocumentError) throw updateDocumentError;
          } else {
            const { error: insertDocumentError } = await admin
              .from("application_documents")
              .insert({
                application_id: application.id,
        ...
        if (batch)
          logs.push({
            batch_id: batch.id,
            external_submission_id: `${source.id}:${rowNumber}`,
            row_number: rowNumber,
            status: "failed",
    ...
    await admin
      ...
      .update({
        worksheet_name: sheet.worksheet.name,
        worksheet_gid: sheet.worksheet.gid,
    ...
    await admin
      ...
      .update({
        ...
        completed_at: new Date().toISOString(),
        records_failed: failed,
        error_details: [{ error: message }, ...errors],
      })
      .eq("id", run.id);
    await admin
      .from("program_data_sources")
      .update({
        ...
        last_sync_summary: { created, updated, skipped, failed, error: message },
      })
      .eq("id", source.id);
    throw error;
  }
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (body.action === "inspect") {
      if (!userId || !body.spreadsheetUrl)
        return json({ error: "An admin session and spreadsheet URL are required." }, 400);
      const spreadsheetId = parseSpreadsheetId(body.spreadsheetUrl);
      if (!spreadsheetId) return json({ error: "Enter a valid Google Sheets URL." }, 400);
      ...
      if (body.dataSourceId) {
        const { data: source } = await admin
          ...
          .single();
        if (!source || !(await isProgramAdmin(admin, userId, source.program_id)))
          return json({ error: "Program administrator access is required." }, 403);
      } else {
        if (!body.programId || !(await isProgramAdmin(admin, userId, body.programId)))
          return json({ error: "Program administrator access is required." }, 403);
      }
    ...
    const query = admin
      .from("program_data_sources")
      .select(
        "id, program_id, spreadsheet_id, spreadsheet_url, worksheet_name, worksheet_gid, sync_enabled, created_by",
      );
    const { data: sources, error } = scheduled
      ? await query.eq("sync_enabled", true)
      : body.dataSourceId
        ? await query.eq("id", body.dataSourceId)
        : { data: null, error: new Error("dataSourceId is required") };
    if (error || !sources) throw error ?? new Error("No data source found.");
    const results = [];
    for (const source of sources as Source[]) {
      if (!scheduled && (!userId || !(await isProgramAdmin(admin, userId, source.program_id))))
        return json({ error: "Program administrator access is required." }, 403);
